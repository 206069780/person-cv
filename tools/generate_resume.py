...
def add_docx_topic(doc: Document, page_no: int, project: dict, topic: dict) -> None:
    add_docx_title(
        doc,
        f"{page_no:02d} / 09 · {project['company']}",
        topic["title"],
    ...
    if project["id"] == "litree":
        p = doc.add_paragraph()
        p.paragraph_format.space_after = Pt(6)
        r = p.add_run("项目事实  国内外 10w+ 水站 · 智慧水务核心微服务与数据工程")
...
def build_docx(data: dict) -> None:
    ...
    if len(pages) != 7:
        raise ValueError(f"Expected 7 project topic pages, got {len(pages)}")
