  ...
  return (
    <group>
      <mesh rotation={[-Math.PI / 2, 0, 0]} receiveShadow>
        ...
        <meshStandardMaterial color="#071015" metalness={0.78} roughness={0.36} />
      </mesh>
      <mesh position={[0, 0.035, -8]} rotation={[-Math.PI / 2, 0, 0]} receiveShadow>
        <circleGeometry args={[3.75, 64]} />
        <meshStandardMaterial color="#09262b" metalness={0.58} roughness={0.18} emissive="#00a89d" emissiveIntensity={0.08} />
      </mesh>
      <mesh position={[0, 0.095, -8]} rotation={[Math.PI / 2, 0, 0]}>
        <torusGeometry args={[4.3, 0.28, 10, 72]} />
        <meshStandardMaterial color="#263b43" metalness={0.94} roughness={0.24} />
      </mesh>
      <mesh position={[0, 0.06, 3]}>
        <boxGeometry args={[0.7, 0.025, 30]} />
        <meshStandardMaterial color="#0a3034" metalness={0.48} roughness={0.2} emissive="#28d7e5" emissiveIntensity={0.12} />
      </mesh>
      {rails.map((x) => (
...
function DataStreams({ motionEnabled, focused }: { motionEnabled: boolean; focused: boolean }) {
  ...
  return (
    <instancedMesh ref={meshRef} args={[undefined, undefined, particles.length]}>
      ...
      <meshBasicMaterial color="#28d7e5" toneMapped={false} transparent opacity={focused ? 0.2 : 0.72} />
    </instancedMesh>
  );
}
