...
class PdfPage:
    ...
    def info_box(self, x: float, y: float, width: float, height: float, label: str, text: str, accent: str = TEAL) -> None:
        ...
        self.wrapped(text, x + 10, y + height - 34, width - 20, size=8.35, leading=11.2, max_lines=max(2, int((height - 31) / 11.2)))
    ...
    def pipeline(self, y: float, flow: str) -> None:
        ...
        for index, node in enumerate(nodes):
            ...
            lines = wrap_pdf_text(node, FONT_BOLD, 7.2, node_width - 10)[:2]
...
def draw_pdf_cover(c: canvas.Canvas, data: dict) -> None:
    ...
    for index, item in enumerate(data["highlights"]):
        ...
        c.setFillColor(HexColor(f"#{COLD_WHITE}"))
        lines = wrap_pdf_text(item, FONT_BOLD, 8.2, 205)
...
def draw_pdf_overview(c: canvas.Canvas, data: dict) -> None:
    ...
    for index, exp in enumerate(data["experiences"]):
        ...
        y = page.wrapped(exp["summary"], timeline_x + 17, y - 2, page.content_width - 24, size=8.25, leading=10.8, max_lines=3) - 13
...
def draw_pdf_topic(c: canvas.Canvas, page_no: int, project: dict, topic: dict) -> None:
    ...
    if project["id"] == "litree":
        fact = "同一 Litree 项目专题展开 · 国内外 10w+ 水站 · " + topic["background"]
    ...
    gap = 8
    col = (page.content_width - gap * 2) / 3
    context = project["businessContext"]
    pain = "；".join(project["painPoints"][:2])
    goals = "；".join(project["buildGoals"][:2])
    page.info_box(page.left, 541, col, 105, "业务场景", context, TEAL)
    page.info_box(page.left + col + gap, 541, col, 105, "现有痛点", pain, ORANGE)
    page.info_box(page.left + (col + gap) * 2, 541, col, 105, "建设目标", goals, CYAN)

    page.section_label(page.left, 521, "业务交付链路", "B2")
    page.pipeline(477, topic["flow"])

    page.section_label(page.left, 460, "职责与工程边界", "B3")
    half = (page.content_width - gap) / 2
    page.info_box(page.left, 371, half, 74, "本人角色", topic["role"], TEAL)
    page.info_box(page.left + half + gap, 371, half, 74, "工程边界", topic["engineeringBoundary"], ORANGE)

    page.section_label(page.left, 351, "工程实现", "B4")
    implementation_width = page.content_width * (0.64 if len(topic["implementation"]) > 4 else 0.5) - gap / 2
    challenge_width = page.content_width - implementation_width - gap
